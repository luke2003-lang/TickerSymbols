...
const JOB_CHUNK_SIZE = 24;
const DEFAULT_POLL_AFTER_MS = 1800;
...
function formatJobMessage(state) {
  ...
  if (state.cooldownUntil) {
    return `Cooling down before retrying ${Math.min(state.pendingNames.length, JOB_CHUNK_SIZE)} unresolved names.`;
  }
...
async function advanceJobState(state) {
  ...
  const now = Date.now();
  const cooldownTime = state.cooldownUntil ? Date.parse(state.cooldownUntil) : 0;
  if (cooldownTime && now < cooldownTime) {
    state.status = 'running';
    state.pollAfterMs = Math.max(cooldownTime - now, 1000);
  ...
  const chunk = state.pendingNames.slice(0, JOB_CHUNK_SIZE);
  ...
  state.pendingNames = lookup.deferredNames.concat(stillPending);

  if (lookup.deferredNames.length) {
    const pollAfterMs = Math.max(lookup.pollAfterMs || RATE_LIMIT_POLL_FLOOR_MS, RATE_LIMIT_POLL_FLOOR_MS);
    state.cooldownUntil = new Date(Date.now() + pollAfterMs).toISOString();
    state.pollAfterMs = pollAfterMs;
    return saveState(state);
  }
