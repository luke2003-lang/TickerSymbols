...
const GEMINI_TAIL_CHUNK_SIZE = JOB_CHUNK_SIZE;
const GEMINI_RETRY_CHUNK_SIZE = JOB_CHUNK_SIZE;
const MAX_GEMINI_DEFER_ATTEMPTS = 5;
...
function formatJobMessage(state) {
  ...
  if (state.geminiCooldownUntil) {
    return `Gemini cooling down with ${state.pendingNames.length} names left to retry.`;
  }

  if (state.massiveCooldownUntil) {
    return `Massive cooling down with ${state.pendingNames.length} names left to retry.`;
  }
...
function getChunkSize(state) {
  ...
  const deferredChunkSize = maxAttempts >= 3 ? GEMINI_RETRY_CHUNK_SIZE : GEMINI_TAIL_CHUNK_SIZE;
  return Math.min(sample.length, deferredChunkSize);
}

function getRateLimitCooldownMs(state, requestedMs) {
  const attempts = state.pendingNames.length
    ? Math.max(...state.pendingNames.map(name => getDeferredAttempts(state, name)), 0)
    : 0;
  const multiplier = Math.max(1, Math.min(4, attempts));
  return Math.max(requestedMs || RATE_LIMIT_POLL_FLOOR_MS, RATE_LIMIT_POLL_FLOOR_MS * multiplier);
}
...
async function finalizeJob(state) {
  ...
  state.pollAfterMs = 0;
  state.massiveCooldownUntil = null;
...
async function advanceJobState(state) {
  ...
  const now = Date.now();
  const massiveCooldownTime = state.massiveCooldownUntil ? Date.parse(state.massiveCooldownUntil) : 0;
  const geminiCooldownTime = state.geminiCooldownUntil ? Date.parse(state.geminiCooldownUntil) : 0;
  const massiveReady = !massiveCooldownTime || now >= massiveCooldownTime;
  ...
  const nextDeferredNames = [];

  for (const name of lookup.deferredNames) {
    const attempts = getDeferredAttempts(state, name) + 1;
    state.deferredAttemptsByName[name] = attempts;

    if (attempts >= MAX_GEMINI_DEFER_ATTEMPTS) {
      state.resultsByName[name] = {
        ticker: 'NEEDS_REVIEW',
      ...
      state.lastProcessedName = name;
      state.recentResults = trimRecentResults([
        { name, ticker: 'NEEDS_REVIEW', source: 'Gemini rate limited' },
        ...state.recentResults
      ]);
      continue;
    }

    nextDeferredNames.push(name);
  }

  state.pendingNames = stillPending.concat(nextDeferredNames);

  if (lookup.massiveRateLimited) {
    const pollAfterMs = Math.max(lookup.massivePollAfterMs || 60000, 15000);
    state.massiveCooldownUntil = new Date(Date.now() + pollAfterMs).toISOString();
  }

  if (lookup.rateLimited) {
    const pollAfterMs = getRateLimitCooldownMs(state, lookup.pollAfterMs);
  ...
  const remainingCooldownMs = state.geminiCooldownUntil
    ...
    : 0;
  const remainingMassiveCooldownMs = state.massiveCooldownUntil
    ? Math.max(Date.parse(state.massiveCooldownUntil) - Date.now(), 0)
    : 0;
  const nextCooldownMs = Math.max(remainingCooldownMs, remainingMassiveCooldownMs);

  if (state.pendingNames.length && nextCooldownMs && !stillPending.length) {
    state.pollAfterMs = Math.max(nextCooldownMs, 1000);
  } else {
