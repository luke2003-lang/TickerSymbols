  ...
  for (const baseUrl of runtime.baseUrls) {
    try {
      ...
      if (response.status === 429) {
        runtime.rateLimited = true;
        runtime.rateLimitMs = MASSIVE_COOLDOWN_MS;
...
export async function resolveBatchDetailed(names, apiKey, options = {}) {
  ...
  const allowMassive = options.allowMassive !== false;
  ...
  const massiveRuntime = massiveApiKey && allowMassive ? createMassiveRuntime(massiveApiKey) : null;
  ...
  if (massiveRuntime && unresolved.length) {
  ...
  if (!unresolvedAfterMassive.length) {
    return {
      ...
      pollAfterMs: 0,
      rateLimited: false,
      massiveRateLimited: Boolean(massiveRuntime?.rateLimited),
      massivePollAfterMs: massiveRuntime?.rateLimitMs || 0
    };
  }
