...
async function searchMassiveTickers(query, market, apiKey) {
  for (const baseUrl of getMassiveBaseUrls()) {
    try {
      ...
      const response = await fetchJson(url.toString(), {}, 10000);
      ...
      if (!response.ok) {
        console.error('Massive lookup failed', { baseUrl, query, market, status: response.status });
        continue;
      }
      ...
      console.error('Massive lookup failed', { baseUrl, query, market, error: error?.message });
    }
  }
...
async function tryMassiveReference(name, apiKey) {
  let bestMatch = null;
  const queries = buildSearchQueries(name).slice(0, 2);

  for (const query of queries) {
    for (const market of ['otc', 'stocks']) {
      const results = await searchMassiveTickers(query, market, apiKey);
...
export async function resolveBatchDetailed(names, apiKey, options = {}) {
  ...
  const massiveConcurrency = Math.max(1, options.massiveConcurrency || 4);
  ...
  if (massiveApiKey) {
    const massiveResults = await mapWithConcurrency(orderedNames, massiveConcurrency, async name => {
      const ticker = await tryMassiveReference(name, massiveApiKey);
      return { name, ticker };
    });
    ...
    unresolvedAfterMassive.push(...orderedNames);
  }

  const unresolved = [];

  const yahooResults = await mapWithConcurrency(unresolvedAfterMassive, yahooConcurrency, async name => {
  ...
  if (!allowGemini) {
    return { results, deferredNames: unresolved, pollAfterMs: 0, rateLimited: false };
  }
